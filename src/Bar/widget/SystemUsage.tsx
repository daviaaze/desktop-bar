...
const disk = new Poll<number>(0, INTERVAL, () => {
  ...
  GTop.glibtop_get_fsusage(diskTop, "/home");
  return diskTop.bavail / diskTop.bfree;
});

const temp = new Poll<number>(
  0,
  INTERVAL,
...
const Indicator = ({
  ...
  <Gtk.LevelBar value={value}>
    ...
    >
      <label label={label} cssClasses={["title"]} />
      <label
        cssClasses={["body"]}
...
export const SystemUsage = () => (
  <button cursor={Gdk.Cursor.new_from_name("pointer", null)}>
    <box spacing={4}>
      <Indicator value={bind(cpu)} label="CPU" unit="%" />
      <Indicator value={bind(memory)} label="RAM" unit="%" />
      <Indicator value={bind(temp)} label="TEMP" vscode-placeholder unit="°C" />
      <Indicator value={bind(disk)} label="DISK" unit="%" />
    </box>
  </button>
);
