...
const getIcon = (client: Hyprland.Client) => {
  switch (client.class) {
    default:
      return (
        apps.fuzzy_query(client.class).at(0)?.iconName ||
...
export const Workspaces = ({ monitor }: { monitor: Hyprland.Monitor }) => (
  <box halign={Gtk.Align.CENTER} valign={Gtk.Align.CENTER}>
    <For
      each={bind(hyprland, "workspaces").as((ws) =>
        ws.filter((ws) => ws.monitor === monitor).sort((a, b) => a.id - b.id)
      )}
    >
      {(ws) => {
        const focusedWs = bind(hyprland, "focusedWorkspace");
        const isFocused = focusedWs.as((focused) => focused === ws);
        const special = ws.id < 0;
        return (
          <Gtk.ToggleButton
            active={bind(hyprland, "focusedWorkspace").as(
              (focused) => focused === ws
            )}
            ...
            $clicked={() => {
              ...
              if (!special && !isFocused.get()) ws.focus();
            }}
          >
